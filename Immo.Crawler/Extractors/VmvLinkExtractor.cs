...
public class VmvLinkExtractor : ILinkExtractorStrategy
{
    public bool CanExtract(string url)
    {
        return url.Contains("vmv-vastgoed.be");
    }
    ...
    public IEnumerable<string> ExtractLinks(string htmlContent, string baseUrl)
    {
        ...
        foreach (var node in anchorNodes)
        {
            ...
            if (!string.IsNullOrWhiteSpace(href) && href.Contains("/te-koop/") && (href.Contains("woning") || href.Contains("bebouwing") || href.Contains("villa"))) 
            {
