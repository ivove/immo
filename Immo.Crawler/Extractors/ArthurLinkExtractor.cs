...
public class ArthurLinkExtractor : ILinkExtractorStrategy
{
    public bool CanExtract(string url)
    {
        return url.Contains("arthur.be") || url.Contains("immotijl.be") || url.Contains("immomiddelpunt.be") || url.Contains("immotroef.be");
    }
    ...
    public IEnumerable<string> ExtractLinks(string htmlContent, string baseUrl)
    {
        ...
        foreach (var node in anchorNodes)
        {
            ...
            if (!string.IsNullOrWhiteSpace(href) && href.Contains("-te-koop-") && (href.Contains("/huis") || href.Contains("/villa"))) 
            {
